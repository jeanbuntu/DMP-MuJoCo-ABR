...
class DMP:
    ...
    def step(self, tau=1.0, k=1.0, start=None, goal=None, actuate=0):
        if goal is None:
            g = self.g
        else:
            g = goal
        ...
        return self.y, self.yd, self.z, self.zd, ag
    ...
    def fit(self, y_demo, tau=1.0):
        ...
        y_demo = it.interpolate_path(self, y_demo)
        yd_demo, ydd_demo = it.calc_derivatives(y_demo, self.dt)

        f_target = tau ** 2 * ydd_demo - self.a * (self.b * (self.g - y_demo) - tau * yd_demo)
        # print(self.g - self.y0)
        ...
        bb = np.sum(bb, axis=1)
        self.w = aa / bb
        # np.savetxt('w_new1', self.w)
        # print('weights is', self.w.shape)
        self.reset()
